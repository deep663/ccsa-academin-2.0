...
const getMarkById = asyncHandler(async (req, res) => {
  const mark = await Mark.find({student: req.params.studentId}) .populate({path: 'student', select:'_id user roll_no', populate: {path: 'user', model: 'User', select: 'name' }}).populate("subject", "_id name");
...
const getMarksBySubjectId = asyncHandler(async (req, res) => {
  ...
  const marks = await Mark.find({subject, course, semester, exam_type}).populate("student subject");
...
const createMark = asyncHandler(async (req, res) => {
  const { title, student, subject, semester, course, marks, total_marks, exam_type } = req.body;

  if (!title && !student || !subject || !semester || !course || !marks || !total_marks || !exam_type) {
    throw new ApiError(400, "All fields are required");
  }
...
const addMultipleMarks = asyncHandler(async (req, res) => {
    const { title, subject, semester, course, marks, total_marks, exam_type } = req.body;
  
    // Validate required fields
    if (!title && !subject || !semester || !course || !marks || !Array.isArray(marks) || !total_marks || !exam_type) {
      throw new ApiError(400, "Invalid input: subject, semester, course, total_marks, exam_type, and marks array are required");
    }
